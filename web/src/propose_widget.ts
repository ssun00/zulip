...
export function activate({any_data}: {any_data: AnyWidgetData; message: any}): {
...
export function render({
  ...
  const deadline_action = is_owner
    ? "Review"
    : i_have_submitted
      ? "Edit"
      : "Submit";
  ...
  void channel.get({
    ...
    success(data) {
      ...
      // show deadline
      $elem
        .find(".propose-deadline-text")
        .text(format_deadline(meeting.deadline));
      ...
      $elem.find(".propose-availability-btn").on("click", () => {
        ...
        const last_start = times.reduce((a, b) => (a > b ? a : b));
        const [lh, lm] = last_start.split(":").map(Number);
        const exclusive_end_mins = lh! * 60 + lm! + slot_duration_mins;
        const end_time = `${String(Math.floor(exclusive_end_mins / 60)).padStart(2, "0")}:${String(exclusive_end_mins % 60).padStart(2, "0")}`;
