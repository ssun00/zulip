...
from zerver.lib.streams import access_stream_common
...
def _get_meeting_or_error(meeting_id: int, user_profile: UserProfile) -> Meeting:
    try:
        meeting = Meeting.objects.select_related("owner", "stream").get(id=meeting_id)
    except Meeting.DoesNotExist:
        raise JsonableError("Meeting not found.")

    # Defensive check: keep error message uniform to avoid information leaks
    # about existence in other realms/streams.
    if meeting.stream.realm_id != user_profile.realm_id:
        raise JsonableError("Meeting not found.")

    # Enforce Zulip stream permissions for private meeting streams.
...
def get_meeting(
    ...
    *,
    ...
    meeting = _get_meeting_or_error(meeting_id, user_profile)
