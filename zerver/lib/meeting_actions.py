...
def do_create_meeting(
    ...
    if create_channel:
        ...
        # Ensure the organizer/owner has access to the private meeting stream.
        # Our permission enforcement requires an active subscription.
        if owner.id not in {u.id for u in invited_users}:
            invited_users.append(owner)
...
def send_meeting_deadline_reminders(*, seconds_before_deadline: int = 3600) -> None:
    """
    Send private reminders to meeting invitees who have not submitted any
    availability responses yet.

    A "non-responder" is defined as: user is subscribed to the meeting's
    stream, is not the meeting owner, and has no MeetingResponse rows for
    any slot in this meeting.
    """
    now = datetime.now(tz=timezone.utc)
    deadline_window_end = now + timedelta(seconds=seconds_before_deadline)
    ...
    meetings = (
        ...
        .select_related("owner", "stream")
    )

    for meeting in meetings:
        subscriber_ids = set(get_subscriber_ids(meeting.stream))
        responder_ids = set(
            MeetingResponse.objects.filter(slot__meeting=meeting).values_list("user_id", flat=True)
        )
        non_responders = subscriber_ids - responder_ids - {meeting.owner_id}
        if not non_responders:
            meeting.reminder_sent_at = now
            meeting.save(update_fields=["reminder_sent_at"])
            continue

        recipients = list(UserProfile.objects.filter(id__in=non_responders))
        deadline_str = meeting.deadline.strftime("%Y-%m-%d %H:%M UTC")
        content = (
            f"Reminder: please submit your availability for **{meeting.topic}** "
            f"(meeting {meeting.id}). Deadline: {deadline_str}.\n\n"
            f"Submit your responses via: PATCH /json/meetings/{meeting.id}/responses"
        )
